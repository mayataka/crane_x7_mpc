...
MPCNodelet::MPCNodelet() 
  : urdf_path_("~/catkin_ws/src/crane_x7_mpc/crane_x7_mpc/IDOCP/examples/cranex7/crane_x7_description/urdf/crane_x7.urdf"),
    robot_(urdf_path_),
    cost_(robot_),
    constraints_(robot_),
    dimq_(robot_.dimq()),
    ...
    Kq_(Eigen::MatrixXd::Zero(robot_.dimq(), robot_.dimq())),
    Kv_(Eigen::MatrixXd::Zero(robot_.dimq(), robot_.dimq())) {
  for (int i=0; i<robot_.dimv(); ++i) {
    joint_efforts_.data.push_back(0);
  }
  robot_.setJointDamping(Eigen::VectorXd::Constant(robot_.dimq(), 1.0e-06));
...
void MPCNodelet::onInit() {
  ...
  timer_ = node_handle_.createTimer(
      ros::Duration(0.005), &cranex7mpc::MPCNodelet::updateControlInputPolicy, 
      this);
  control_input_policy_publisher_
      = node_handle_.advertise<crane_x7_mpc::ControlInputPolicy>(
...
void MPCNodelet::updateControlInputPolicy(const ros::TimerEvent& time_event) {
  ...
  mpc_.getStateFeedbackGain(Kq, Kv);
  Eigen::Map<Eigen::VectorXd>(control_input_policy.q, dimq_) = q_;
  Eigen::Map<Eigen::VectorXd>(control_input_policy.v, dimv_) = v_;
  Eigen::Map<Eigen::VectorXd>(control_input_policy.u, dimv_) = u_;
  Eigen::Map<Eigen::MatrixXd>(control_input_policy.Kq, dimv_, dimv_) = Kq_;
  Eigen::Map<Eigen::MatrixXd>(control_input_policy.Kv, dimv_, dimv_) = Kv_;
  control_input_policy_publisher_.publish(control_input_policy);
}
